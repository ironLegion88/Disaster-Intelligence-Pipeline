...
def load_processed_data():
    df = pd.read_csv("data/processed/clean_metrics.csv")
    return df
...
*   **Accelerated Mobilization (Delta Time):** Is the world responding faster today? **Yes.** The time gap between the GDACS System Alert and Peak Media Volume has reduced significantly. In 2021 (Haiti), the media took **{df.iloc[0]['Response_Delta_Days']} days** to peak. By 2025 (Myanmar), response time improved to **{df.iloc[1]['Response_Delta_Days']} days**. The deployment of rapid-assessment technologies, evidenced by the extraction of spatial-tech entities like **"ISRO"** (Indian Space Research Organisation) in the Myanmar news data, fuels this acceleration.
*   **The "Forgotten Crisis" & Media Bias:** Despite Myanmar generating a higher absolute volume of news ({df.iloc[1]['Total_News_Articles']} articles vs Haiti's {df.iloc[0]['Total_News_Articles']}), our **Forgotten Crisis Index** reveals a stark disparity in *proportional* coverage. Haiti's index was **{df.iloc[0]['Forgotten_Crisis_Index']}**, whereas Myanmar's is drastically lower at **{df.iloc[1]['Forgotten_Crisis_Index']}**. Given that Myanmar had over 17.2 million people exposed in the MMI>=VII regions, nearly 18 times more than Haiti's 932,435, the media coverage for Myanmar is severely disproportionate to the human impact. Myanmar is definitively a "forgotten crisis" on a per-capita exposure basis.
*   **Vulnerability & Coping Capacity:** The coping capacity metrics mirror the tragic outcomes. Myanmar registered an extreme vulnerability score of **{df.iloc[1]['Vulnerability_Score']}** and a negative coping capacity (**{df.iloc[1]['Estimated_Coping_Capacity']}**), reflecting a systemic inability to absorb a 7.7 Magnitude shock. This mathematical fragility directly corroborates with the NLP entity extraction across the entire headline corpus, which detected immediate alarming casualty figures like **"10,000 Casualties"** in the Myanmar dataset. In contrast, Haiti's reports, while tragic with **"2,189 dead"**, occurred in a context of slightly higher coping capacity.
